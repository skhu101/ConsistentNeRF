...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, depth_pred, _ = render(H, W, K, chunk=chunk, c2w=c2w[:3,:4], **render_kwargs)
        rgbs.append(rgb.cpu().numpy())
        disps.append(disp.cpu().numpy())
        depth_preds.append(depth_pred.cpu().numpy())
        if i==0:
    ...
    depth_preds = np.stack(depth_preds, 0)

    return rgbs, disps, depth_preds
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split, depths_cas = load_blender_view_data(args.datadir, args.half_res, args.testskip, train_view_num=args.train_view_num)
        print('Loaded blender', images.shape, depths_cas.shape, render_poses.shape, hwf, args.datadir)
        name = os.path.basename(args.datadir)
        i_train_aug = torch.load('configs/pairs.th')[f'{name}_train'][:args.train_view_num]
    ...
    for tgt_index in range(images.shape[0]):
        ...
        imageio.imwrite(f'{basedir}/{expname}/mask/{scene}/{args.train_view_num}view/{tgt_index}_mask_{args.train_view_num}view.jpg', (255-mask_all[-1].reshape(H, W) * 255).astype(np.uint8))
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _, depth_preds = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=images, savedir=testsavedir, render_factor=args.render_factor)
            import pdb
            pdb.set_trace()
    ...
    for i in range(start, N_iters):
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs_test, disps_test = render_path(torch.Tensor(poses[i_test]).to(device), hwf, K, args.chunk, render_kwargs_test, gt_imgs=images_label[i_test], savedir=testsavedir)
