...
def get_ref_rays(w2c_ref, c2w_ref, intrinsic_ref, point_samples, img, depths_h=None):
    ...
    if w2c_ref is not None:
        ...
        R = w2c_ref[:, :3, :3]  # (B, 3, 3)
        T = w2c_ref[:, :3, 3:]  # (B, 3, 1)
        point_samples = point_samples @ R.permute(0,2,1) + T.reshape(B, 1, 3) # [B, ray_samples*N_samples, 3] [5, 131072, 3]
    ...
    if depths_h is not None:
        ...
        return rgb_ref, depth_h_ref, point_samples[mask], rays_o, rays_d, mask
    else:
        return rgb_ref, point_samples[mask], rays_o, rays_d, mask
...
def config_parser():
    ...
    parser.add_argument('--ss_loss', action='store_true')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split = load_blender_view_data(args.datadir, args.half_res, args.testskip, train_view_num=args.train_view_num)
        ...
        images, poses, bds, render_poses, hwf, depths_cas, depths = load_dtu_data(args.datadir)
        ...
        for i in i_train:
            rgb8 = to8b(images[i])
            filename = os.path.join('images_test/', '{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
    ...
    for i in range(start, N_iters):
        ...
        if args.ss_loss:
            ...
            ref_index = np.random.choice(i_train) #random.randint(0, 2)
            # while ref_index == img_i:
            #     ref_index = np.random.choice(i_train)
            c2w_ref = torch.eye(4)
            c2w_ref[:3, :4] = poses[ref_index]
            ...
            while mask.sum() == 0:
                depth_diff = point_samples_c_ref[...,-1].unsqueeze(-1)-rays_depth_ref.squeeze()[:, None]
                mask = (abs(depth_diff)<occlusion_threshold)
                occlusion_threshold = 2 * occlusion_threshold


            batch_rays_ref = torch.stack([rays_o_ref, rays_d_ref], 0)

            rgb_ref, disp_ref, acc_ref, depth_pred_ref, extras_ref = render(H, W, K, chunk=args.chunk, rays=batch_rays_ref,
                                                    verbose=i < 10, retraw=True,
                                                    **render_kwargs_train)

            img_loss_ref = img2mse(rgb_ref, rgb_target_ref.squeeze().permute(1,0))
            loss += img_loss_ref
            if args.with_depth_loss:
                loss += img2mse(depth_pred_ref, rays_depth_ref)

            if 'rgb0' in extras_ref:
                loss += img2mse(extras_ref['rgb0'], rgb_target_ref.squeeze().permute(1,0))
                if args.with_depth_loss:
                    loss += img2mse(extras_ref['depth0'], rays_depth_ref)


        optimizer.zero_grad()
        if args.ss_loss:
            img_loss = img2mse(rgb[mask_bound.squeeze()][mask.squeeze()], target_s[mask_bound.squeeze()][mask.squeeze()]) if random.randint(0, 1) == 1 else img2mse(rgb, target_s)
        else:
        ...
        if args.with_depth_loss:
            if args.ss_loss:
                loss += img2mse(depth_pred[mask_bound.squeeze()][mask.squeeze()],
                                depth_cas_s[mask_bound.squeeze()][mask.squeeze()]) if random.randint(0, 1) == 1 else 0
            else:
                loss += img2mse(depth_pred, depth_cas_s)

        if 'rgb0' in extras:
            if args.ss_loss:
                img_loss0 = img2mse(extras['rgb0'][mask_bound.squeeze()][mask.squeeze()],target_s[mask_bound.squeeze()][mask.squeeze()]) if random.randint(0,1) == 1 else img2mse(rgb, target_s)
            else:
            ...
            if args.with_depth_loss:
                if args.ss_loss:
                    loss += img2mse(extras['depth0'][mask_bound.squeeze()][mask.squeeze()], depth_cas_s[mask_bound.squeeze()][mask.squeeze()]) if random.randint(0, 1) == 1 else 0
                else:
                    loss += img2mse(extras['depth0'], depth_cas_s)
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            test_loss = img2mse(torch.Tensor(rgbs_test), torch.Tensor(images[i_test]))
            test_psnr = mse2psnr(test_loss)
            print('test psnr: ', test_psnr)
            ...
            for i in range(rgbs_test.shape[0]):
                mask = depths_test[i]==0
                rgbs_test_mask[i][mask] = 0
                images_mask[i][mask] = 0

            test_loss = img2mse(rgbs_test_mask, images_mask)
            test_psnr = mse2psnr(test_loss)
            print('test mask psnr: ', test_psnr)
