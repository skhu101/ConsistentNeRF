...
def train_step(
  ...
  def loss_fn(variables):
    ...
    fract1 = tree_sum(jax.tree_map(lambda z: jnp.sum(z**2), variables))
    fract2 = tree_sum(
            jax.tree_map(lambda z: jnp.prod(jnp.array(z.shape)), variables))
    
    weight_l2 =  jax.numpy.nan_to_num(jnp.where(fract2 == 0, 0, fract1/fract2))
    ...
    for rendering in renderings:
      ...
      denom = lossmult.sum()
      losses.append(numer / (denom + 0.0001))
      
      if config.compute_depth_metrics:
        if config.dataset_loader == 'dtu' or config.use_hardmask:
          mask = batch['masks']         
          #alpha = (rendering['distance_mean']*mask - batch['dpts']*mask).mean()*config.batch_size/mask.sum()
          if config.dataset_loader == 'llff':
            depth_mse = ((batch['dpts']*mask/batch['bounds'].max() - rendering['distance_mean']*mask/batch['bounds'].max())**2).mean()
          
          elif config.dataset_loader == 'blender':
            depth_mse = ((batch['dpts']*mask/config.far - rendering['distance_mean']*mask/config.far)**2).mean()
          else:
            depth_mse = ((batch['dpts']*mask - rendering['distance_mean']*mask)**2).mean()
          depth_mses.append(depth_mse)
        
        else:
          
          if config.dataset_loader == 'llff':
            depth_mse = ((batch['dpts']/batch['bounds'].max() - rendering['distance_mean']/batch['bounds'].max())**2).mean()
          else:
            depth_mse = ((batch['dpts'] - rendering['distance_mean'])**2).mean()
          depth_mses.append(depth_mse)
      else:
        depth_mses.append(0.0)
      #disp_mses.append(rendering['distance_mean'].min())
      #disp_mses.append(rendering['distance_mean'].max())
      #disp_mses.append(rendering['distance_mean'].mean())
      
      patch_num = 4
      if config.compute_mono_depth_metrics:
        depth_predict_clip = 1 / jnp.where(rendering['distance_mean'] <= 0, 0.001, rendering['distance_mean'])
        # depth_predict = rendering['distance_mean'][:config.batch_size]
        # depth_gt = batch['dpts'][:config.batch_size]
        depth_mse = 0.0
        for i in range(patch_num):
          depth_predict = jax.numpy.nan_to_num(depth_predict_clip[i * 16*16 : (i+1) * 16*16])
          depth_gt = jax.numpy.nan_to_num(batch['mono_dpts'][i * 16*16 : (i+1) * 16*16])  

          mask = jnp.where(depth_gt>0, 1, 0)
          depth_min = jnp.where(depth_gt > 0, depth_gt, 10**5).min()
          depth_max = depth_gt.max()
          depth_gt = mask * (depth_gt - depth_min) / (depth_max - depth_min + 0.0001)

          depth_min = jnp.where(mask * depth_predict>0, depth_predict, 10**5).min()
        ...
        if config.dataset_loader == 'blender':
          depth_mse *= 0.01
        mono_depth_mses.append(depth_mse)
      else:
        mono_depth_mses.append(0.0)
    ...
    depth_mses = jnp.array(depth_mses)
    mono_depth_mses = jnp.array(mono_depth_mses)
    ...
    loss = (
        ...
        0.1 * depth_mses[-1] + 0.1 * config.coarse_loss_mult * jnp.sum(depth_mses[:-1]) +
        0.1 * mono_depth_mses[-1] + 0.1 * config.coarse_loss_mult * jnp.sum(mono_depth_mses[:-1]) +
    ...
    return loss, (losses, depth_mses, mono_depth_mses, disp_mses, normal_maes, weight_l2, losses_georeg)

  (loss, loss_aux), grad = (jax.value_and_grad(loss_fn, has_aux=True)(
      state.optimizer.target))
  (losses, depth_mses, mono_depth_mses, disp_mses, normal_maes, weight_l2, losses_georeg) = loss_aux
  grad = jax.lax.pmean(grad, axis_name='batch')
  losses = jax.lax.pmean(losses, axis_name='batch')
  depth_mses = jax.lax.pmean(depth_mses, axis_name='batch')
  mono_depth_mses = jax.lax.pmean(mono_depth_mses, axis_name='batch')
  ...
  stats = TrainStats(
      loss=loss,
      losses=losses,
      depth_mses=depth_mses,
      mono_depth_mses = mono_depth_mses,
  ...
  for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
    ...
    if jax.host_id() == 0:
      ...
      if step % config.print_every == 0:
        ...
        for i, l in enumerate(stats.depth_mses):
          summary_writer.scalar(f'train_depth_mse_losses_{i}', l, step)     
        for i, l in enumerate(stats.mono_depth_mses):
          summary_writer.scalar(f'train_mono_depth_mse_losses_{i}', l, step)      
        ...
        print(f'{step:{precision}d}' + f'/{config.max_steps:d}: ' +
              ...
              f'rgb_coarse ={stats.losses[0]:0.4f}, ' + f'rgb_fine ={stats.losses[-1]:0.4f}, ' +
              f'monodpt_coarse={stats.mono_depth_mses[0]:0.4f}, ' +
              f'monodpt_fine={stats.mono_depth_mses[-1]:0.4f}, ' +
              f'depth_coarse_mse={stats.depth_mses[0]:0.4f}, ' +
              f'depth_fine_mse={stats.depth_mses[-1]:0.4f}, ' +
              f'weight_l2={stats.weight_l2:0.2e}, ' +
              f'train_grad_norm={stats.grad_norm:0.4f}, ' +
              f'train_grad_abs_max={stats.grad_abs_max:0.4f}, ' +
              #f'geo_coarse_={stats.losses_georeg[0]:0.4f}, ' +
              #f'geo_fine={stats.losses_georeg[-1]:0.4f}, ' +
    ...
    if config.train_render_every > 0 and step % config.train_render_every == 0:
      ...
      if jax.host_id() == 0:
        ...
        if not config.render_path:
          ...
          import cv2
          ind = np.array([2,1,0])
          pred = np.asarray(rendering['rgb'])
          gt = np.asarray(test_case['rgb'])
          cv2.imwrite(f'{config.checkpoint_dir}/rgb_pred.png',(pred[:,:,ind])*255)
          cv2.imwrite(f'{config.checkpoint_dir}/rgb_gt.png',(gt[:,:,ind])*255)
          
          #添加depth可视化
          
          depth = 1/ rendering['distance_mean']
          depth_min = depth.min()
          depth_max = depth.max()
          normalized_depth = 255 * (depth - depth_min) / (depth_max - depth_min)
          normalized_depth *= 3
          right_side = jax.numpy.repeat(jax.numpy.expand_dims(normalized_depth, 2), 3, axis=2) / 3
          right_side = cv2.applyColorMap(np.uint8(right_side), cv2.COLORMAP_INFERNO)
          cv2.imwrite(f'{config.checkpoint_dir}/depth_pred.png', right_side)
          
          #ipdb.set_trace()
          if config.dataset_loader == 'dtu'and not config.dtu_no_mask_eval:
            gt = test_case['dpts']
            mask = test_case['masks']
            depth_max = 1/gt[gt != 0].min()
            depth_min = 1/gt[gt != 0].max()
            gt = 1/ (test_case['dpts']+ 1e-6)
            gt = gt*mask
            
            normalized_depth = 255 * (gt - depth_min) / (depth_max - depth_min)
            normalized_depth *= 3
            right_side = jax.numpy.repeat(jax.numpy.expand_dims(normalized_depth, 2), 3, axis=2) / 3
            right_side = cv2.applyColorMap(np.uint8(right_side), cv2.COLORMAP_INFERNO)
            cv2.imwrite(f'{config.checkpoint_dir}/depth_gt.png', right_side)
