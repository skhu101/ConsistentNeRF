...
class Config:
  ...
  batching: str = 'single_image_patch'  # Batch composition.
  ...
  batch_size_random: int = 2048  # The number of rays/pixels in each batch.
  ...
  render_path: bool = True  # If True, render a path. Used by LLFF and DTU.
  render_train: bool = False  # If True, renders train images instead.
  render_path_frames: int = 120  # Number of frames in path. Used only by LLFF.
  ...
  compute_depth_metrics: bool = False#If True, load and compute depth loses
  compute_mono_depth_metrics: bool = True 
    #If True, use midas depth to compute a loss. Note that even this parameter is false, MiDaS depth will still be loaded.
  
  use_hardmask: bool = True #If True for llff/blender dataset, load hardmask to compute depth_mse.
  use_nerf_depth: bool = False#If True, load flow depth instead of gt depth and use hardmask instead of gt mask, dtu only
  ...
  max_steps: int = 50002  # The number of optimization steps.
  checkpoint_every: int = 25000  # The number of steps to save a checkpoint.
  print_every: int = 100  # The number of steps between reports to tensorboard.
  train_render_every: int = 2000  # Steps between test set renders for training
  n_input_views: int = 9  # Restrict the number of input views.
  n_random_poses: int = 1000  # How many random poses to use.
...
def load_config(save_config=True):
  ...
  config = Config()
  if save_config and jax.host_id() == 0 and not os.path.isdir(config.checkpoint_dir):
