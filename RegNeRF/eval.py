  ...
  while True:
    ...
    for idx in range(dataset.size):
      ...
      if not config.render_path:
        ...
        #添加lpips计算
        gt = np.asarray(rendering['rgb'])
        gt = torch.from_numpy(gt)
        pred = np.asarray(batch['rgb'])
        pred = torch.from_numpy(pred)
        gt, pred = (gt-0.5)*2., (pred-0.5)*2.
        gt, pred = gt.permute([2,0,1]),pred.permute([2,0,1])
        #ipdb.set_trace()
        metric['lpips'] = float(lpips_fn(gt,pred).item())
        ...
        if config.dataset_loader == 'dtu'and not config.dtu_no_mask_eval:
          ...
          dpt_gt = rendering['distance_mean']
          dpt_pred = batch['dpts']
          metric['depth_mse_masked'] = ((dpt_gt[mask] - dpt_pred[mask])**2).mean()
          
          #convert to (512,640,3)
          #ipdb.set_trace()
          ...
          #添加lpips计算
          gt = np.asarray(rgb_hat_fg)
          gt = torch.from_numpy(gt)
          pred = np.asarray(rgb_fg)
          pred = torch.from_numpy(pred)
          gt, pred = (gt-0.5)*2., (pred-0.5)*2.
          gt, pred = gt.permute([2,0,1]),pred.permute([2,0,1])
          metric['lpips_masked'] = float(lpips_fn(gt, pred).item())
      ...
      if config.eval_save_output and (config.eval_render_interval > 0):
        if (idx % config.eval_render_interval) == 0:
          ...
          '''
            ...
            re_ind = np.array([2,1,0])
            cv2.imwrite(path_fn(f'depth_pred_{idx:03d}.png'), right_side[:,:,re_ind])
          '''
          aa=vis.visualize_depth(rendering['distance_mean'], rendering['acc'])
          utils.save_img_u8(aa, path_fn(f'depth_{idx:03d}.png'))
          
          
          if config.dataset_loader == 'dtu'and not config.dtu_no_mask_eval and not config.render_path:
            gt = batch['dpts']
            mask = batch['masks']
            depth_max = 1/gt[gt != 0].min()
            depth_min = 1/gt[gt != 0].max()
            gt = 1/ (batch['dpts']+ 1e-6)
            gt = gt*mask
            ...
            right_side = jax.numpy.repeat(jax.numpy.expand_dims(normalized_depth, 2), 3, axis=2) / 3
            right_side = cv2.applyColorMap(np.uint8(right_side), cv2.COLORMAP_INFERNO)
            cv2.imwrite(path_fn(f'depth_gt_{idx:03d}.png'), right_side)
